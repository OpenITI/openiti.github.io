...
def clean_md_update_image_routing(blog_text):
      ...
      blog_text = re.sub(r"\^([^\^]+)\^", r"<sup>\1</sup>", blog_text)
...
def take_add_excerpt(header_dict, blog_text, excerpt_w_len = 50):
  blog_words = blog_text.split(" ")
  if len(blog_words) > excerpt_w_len:
    excerpt = " ".join(blog_words[:excerpt_w_len]) + "..."
  else:
    excerpt = blog_text[:]
  header_dict["excerpt"] = excerpt
  return header_dict
    ...
    if docx_check.match(docx_path):
        ...
        if blog_text is None:
           print("WARNING - {} - file not found - have you uploaded the corresponding docx file?".format(docx_path))
            ...
            if add_excerpt:
               header = take_add_excerpt(header, blog_text)           
